...
def create_app() -> FastAPI:
    ...
    def _startup() -> None:
        # Не роняем весь сайт, если база временно недоступна: логируем и продолжаем,
        # чтобы страница хотя бы открывалась (а причина видна в логах).
        try:
            init_db()
        except Exception as exc:  # pragma: no cover
            print(f"[startup] init_db failed: {exc!r}")
